...
@section('title')
    <title>Edit Payment - Harvest Vape</title>
    ...
    <div class="w-full px-32">
        <div class="card">
            <div class="card-body p-4 mt-20">
                ...
                <p class="-mt-8 mb-5 font-semibold" style="font-size: 15px">Pembayaran {{ $payment->kode_payment }}</p>

                <form method="POST" action="{{ route('admin.payment.update', ['id' => $payment->id]) }}">
                    @csrf
                    @method('PUT')
                    <div class="d-flex">
                        <div class="flex-fill me-3">
                            <div class="mb-3">
                                ...
                                <input type="date" class="form-control @error('tanggal_payment') is-invalid @enderror"
                                    id="tanggal_payment" name="tanggal_payment"
                                    value="{{ old('tanggal_payment', $payment->tanggal_payment) }}">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('ket_payment') is-invalid @enderror" id="ket_payment"
                                    ...
                                    <option value="Listrik, Air, dan Telepon"
                                        {{ old('ket_payment', $payment->ket_payment) == 'Listrik, Air, dan Telepon' ? 'selected' : '' }}>
                                        Listrik, Air, dan Telepon</option>
                                    <option value="Gaji Karyawan"
                                        {{ old('ket_payment', $payment->ket_payment) == 'Gaji Karyawan' ? 'selected' : '' }}>
                                        Gaji Karyawan</option>
                                    <option value="Sewa"
                                        {{ old('ket_payment', $payment->ket_payment) == 'Sewa' ? 'selected' : '' }}>Sewa
                                    </option>
                                    <option value="Pajak"
                                        {{ old('ket_payment', $payment->ket_payment) == 'Pajak' ? 'selected' : '' }}>Pajak
                                    </option>
                                    <option value="Asuransi"
                                        {{ old('ket_payment', $payment->ket_payment) == 'Asuransi' ? 'selected' : '' }}>
                                        Asuransi</option>
                                    <option value="Pemasaran"
                                        {{ old('ket_payment', $payment->ket_payment) == 'Pemasaran' ? 'selected' : '' }}>
                                        Pemasaran</option>
                                    <option value="Bunga"
                                        {{ old('ket_payment', $payment->ket_payment) == 'Bunga' ? 'selected' : '' }}>Bunga
                                    </option>
                                    <option value="Depresiasi"
                                        {{ old('ket_payment', $payment->ket_payment) == 'Depresiasi' ? 'selected' : '' }}>
                                        Depresiasi</option>
                                    {{-- <option value="Pengiriman"
                                        {{ old('ket_payment', $payment->ket_payment) == 'Pengiriman' ? 'selected' : '' }}>
                                        Pengiriman</option> --}}
                                </select>
                                <div class="invalid-feedback">
                                    @error('ket_payment')
                        ...
                        <div class="flex-fill">
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control @error('cost_payment') is-invalid @enderror"
                                    ...
                                    value="{{ old('cost_payment', $payment->cost_payment) }}">
                                <div class="invalid-feedback">
                                    @error('cost_payment')
                                        {{ $message }}
                                    @enderror
